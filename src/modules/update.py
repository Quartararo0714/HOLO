...
class CNN_64(nn.Module):
    def __init__(self, input_dim=256, indim=164):
        super(CNN_64, self).__init__()

        outputdim = input_dim
        self.layer1 = nn.Sequential(nn.Conv2d(indim, outputdim, 3, padding=1, stride=1),
                                    nn.GroupNorm(num_groups=outputdim//8, num_channels=outputdim), nn.ReLU(), nn.MaxPool2d(kernel_size = 2, stride=2))
        input_dim = outputdim
        outputdim = input_dim
        self.layer2 = nn.Sequential(nn.Conv2d(input_dim, outputdim, 3, padding=1, stride=1),
...
class GMA(nn.Module):
    def __init__(self, sz, in_dim = 164):
        ...
        if sz==32:
            self.cnn = CNN(128)
        if sz==64:
            self.cnn = CNN_64(128, indim=in_dim)
...
class GMA_update(nn.Module):
    def __init__(self, sz, in_dim = 128):
        super().__init__()
        if sz==64:
            self.cnn = CNN_64(64, indim=in_dim)
